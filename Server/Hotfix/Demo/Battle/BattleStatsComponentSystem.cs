namespace ET.Server
{
    ...
    public static partial class BattleStatsComponentSystem
    {
        ...
        public static void SetCore(this BattleStatsComponent self, int hp, int maxHp, int attack, int defense, float speed, bool mirrorNumeric)
        {
            ...
            // 过渡期保留 Numeric 镜像，避免旧广播和调试代码读不到最新值。
            NumericComponent numeric = self.GetParent<BattleUnit>()?.GetComponent<NumericComponent>();
            numeric?.Set(NumericType.Hp, hp);
            numeric?.Set(NumericType.MaxHp, maxHp);
            numeric?.Set(NumericType.Attack, attack);
            numeric?.Set(NumericType.Defense, defense);
        ...
        public static void SetHpMax(this BattleStatsComponent self, int hp, int maxHp, bool mirrorNumeric)
        {
            ...
            // 血量下发仍依赖 NumericComponent 的旧字段，快照和 Numeric 暂时同步维护。
            NumericComponent numeric = self.GetParent<BattleUnit>()?.GetComponent<NumericComponent>();
            numeric?.Set(NumericType.MaxHp, self.MaxHp);
            numeric?.Set(NumericType.Hp, self.Hp);
        }
        ...
        public static void SetHp(this BattleStatsComponent self, int hp, bool mirrorNumeric)
        {
            ...
            if (mirrorNumeric)
            {
                self.GetParent<BattleUnit>()?.GetComponent<NumericComponent>()?.Set(NumericType.Hp, self.Hp);
            }
        }
